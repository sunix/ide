...
public class ResetToCommitPresenterTest extends BaseTest {
    ...
    @Test
    public void testOnResetClickedWhenFileIsNotExistInCommitToResetAndDiffAndResetRequestsIsSuccessful() throws Exception {
        doAnswer(new Answer() {
            @Override
            public Object answer(InvocationOnMock invocation) throws Throwable {
                Object[] arguments = invocation.getArguments();
    ...
    @Test
    public void testOnResetClickedWhenFileIsChangedInCommitToResetAndDiffAndResetRequestsIsSuccessful() throws Exception {
        doAnswer(new Answer() {
            @Override
            public Object answer(InvocationOnMock invocation) throws Throwable {
                Object[] arguments = invocation.getArguments();
    ...
    public void testOnResetClickedWhenDiffRequestIsFailed() throws Exception {
        ...
        verify(service).diff(eq(PROJECT_ID), anyList(), eq(DiffRequest.DiffType.RAW), eq(true), eq(0), eq(PROJECT_ID), eq(false),
                             (AsyncRequestCallback<String>)anyObject());
        verify(service, never()).reset(anyString(), anyString(), (ResetRequest.ResetType)anyObject(), (AsyncRequestCallback<Void>)anyObject());
    }
